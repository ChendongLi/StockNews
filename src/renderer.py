...
def build_html(
    ...
    for ticker, name in stocks.items():
        ...
        summary_box = (
            ...
            f'<p style="margin:0;color:#1e293b;font-size:14px;line-height:1.6">{summary}</p>'
            f"</div>"
        ) if summary else ""
        ...
        if not items:
            ...
            for item in items:
                ...
                rows += (
                    ...
                    f'<a href="{item["link"]}" style="color:#1d4ed8;font-weight:600;'
                    'font-size:14px;text-decoration:none;line-height:1.4">'
                    f'{item["title"]}</a>'
                    f'<br><span style="color:#6b7280;font-size:11px">{item["published"]}</span>'
                    + (
        ...
        sections += (
            ...
            f'<h2 style="margin:0 0 14px;font-size:19px;color:#0f172a">{badge} &nbsp;{name}</h2>'
            f"{summary_box}"
...
<div style="max-width:700px;margin:0 auto;background:#fff;border-radius:16px;
  ...
  <div style="padding:36px">{sections}
    <p style="color:#9ca3af;font-size:11px;text-align:center;margin-top:8px">
      Powered by Yahoo Finance RSS and Claude AI
    </p>
  </div>
</div>
</body></html>"""
